...
function getSpriteDisplay(runtime: BuddyEditorRuntime): {
	...
	const spriteLeftPad = Math.max(
		0,
		Math.floor((displayWidth - visualSpriteWidth) / 2),
	);
	...
	const lines = [
		...
		...trimmedLines.map((l) =>
			(" ".repeat(spriteLeftPad) + l).padEnd(displayWidth),
		),
...
export class BuddyEditor extends CustomEditor {
	...
	constructor(
		...
		this.animTimer = setInterval(() => {
			...
			visual.tick += 1;
			if (visual.bubbleUntil && Date.now() > visual.bubbleUntil)
				visual.bubbleText = null;
			if (visual.animationState === "petted" && Date.now() > visual.heartsUntil)
				visual.animationState = "idle";
...
export function installBuddyEditor(
	_pi: ExtensionAPI,
	ctx: any,
	runtime: BuddyEditorRuntime,
): void {
	...
	void ctx.ui.custom<void>(
		(
			...
			_theme: any,
			_keybindings: any,
			_done: (result: void) => void,
		) => new BuddySpriteOverlay(runtime),
		{
			overlay: true,
			overlayOptions: {
				anchor: "bottom-right" as const,
				width: runtime.getActiveBuddy()
					? getBuddyDisplayWidth(runtime.getActiveBuddy()!)
					: 0,
				margin: { right: 1, bottom: 2 },
				nonCapturing: true,
				visible: (termWidth: number) =>
					termWidth >= 60 && getSpriteDisplay(runtime).visible,
			},
			onHandle: (handle) => {
