...
export function canUseModelSoul(state: BuddyState, hasModel: boolean): boolean {
  ...
  return state.sessionUsage.buddyModelCalls < state.settings.maxBuddyModelCallsPerSession;
}
...
export function canUseModelReaction(input: ReactionGateInput): boolean {
  ...
  if (!state.settings.reactionEnabled || state.settings.hidden || state.settings.muted) return false;
  if (state.settings.reactionMode !== 'cheap-model') return false;
  if (state.sessionUsage.reactionCalls >= state.settings.maxReactionCallsPerSession) return false;
  if (state.sessionUsage.buddyModelCalls >= state.settings.maxBuddyModelCallsPerSession) return false;
  if (input.completedTurns - input.lastReactionTurn < TOKEN_POLICY.reactionTurnCooldown) return false;
  if (now - input.lastReactionAt < TOKEN_POLICY.reactionTimeCooldownMs) return false;
  return true;
}

export function recordModelUsage(state: BuddyState, inputTokens: number, outputTokens: number, kind: 'soul' | 'reaction'): void {
